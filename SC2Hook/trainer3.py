...
def compute_reward(exp: np.ndarray, prev_self_err: float) -> float:
    ...
    if done:
       reward += 0.0 if stock_delta > 0 else -5.0

    # Self-modeling
    err_delta = prev_self_err - self_err
    if err_delta > 0:
        reward += 0.0 * err_delta * 100
    reward -= self_err * 0.00

    if mem_hit > 0.7:
        reward += 0.0 * mem_hit

    # Strong button penalties
    ...
    btn_sum = btn_actions.sum()
   # if btn_sum > 0:
    #    reward -= btn_sum * 0.04 + (btn_sum ** 2) * 0.025

    if int(btn_actions.sum()) == 0 and mem_op == 0:
        reward -= 0.0
...
def main():
    ...
    if os.path.exists(CHECKPOINT_PATH):
        print(f" Loading PPO checkpoint: {cyan(CHECKPOINT_PATH)}")
        model.load_state_dict(torch.load(CHECKPOINT_PATH, map_location=device), strict=False)
    elif os.path.exists(WEIGHTS_PATH):
        print(f" Loading supervised weights: {cyan(WEIGHTS_PATH)}")
        model.load_state_dict(torch.load(WEIGHTS_PATH, map_location=device), strict=False)
    else:
        print(f" {yellow('No checkpoint — starting from random init')}")
    ...
    try:
        ...
        if os.path.exists(BUFFER_PATH):
            buf_mmap._open()
            buffer = buf_mmap
            print(f" {green('Connected to live runner buffer')}\n")
        else:
            print(f" {yellow('Using in-process fallback')}\n")
            buffer = InProcessBuffer(capacity=BUFFER_CAPACITY)
    except Exception as e:
        print(f" {yellow(f'Buffer failed: {e}')} — using fallback\n")
        buffer = InProcessBuffer(capacity=BUFFER_CAPACITY)
    ...
    while running:
        ...
        if frames_since_update >= UPDATE_INTERVAL and len(buffer) >= BATCH_SIZE * 2:
            ...
            for i in range(n_sample):
                r = compute_reward(batch[i], prev_self_err)
                rewards[i] = r
